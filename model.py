...
class pix2pix(object):
    ...
    def build_model(self):
        ...
        self.real_AB = tf.concat(values=[self.real_A, self.real_B], axis=3)
        self.fake_AB = tf.concat(values=[self.real_A, self.fake_B], axis=3)
        ...
        # classfier loss
        gen_img = tf.image.resize_images(self.fake_B, (299, 299))
    ...
    def train(self, args):
        ...
        for epoch in xrange(args.epoch):
            ...
            # alternative training data set (real photo/painting edge)
            alt_data = glob('./datasets/{}/train_alt/*.jpg'.format(self.dataset_name))
            alt_batch_idxs = min(len(alt_data), args.train_size)
    ...
    def generator(self, image, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            d1 = tf.concat(values=[d1, e7], axis=3)
            # d1 is (2 x 2 x self.gf_dim*8*2)

            self.d2, self.d2_w, self.d2_b = deconv2d(tf.nn.relu(d1),
                [self.batch_size, s64, s64, self.gf_dim*8], name='g_d2', with_w=True)
            d2 = tf.nn.dropout(self.g_bn_d2(self.d2), 0.5)
            d2 = tf.concat(values=[d2, e6], axis=3)
            # d2 is (4 x 4 x self.gf_dim*8*2)

            self.d3, self.d3_w, self.d3_b = deconv2d(tf.nn.relu(d2),
                [self.batch_size, s32, s32, self.gf_dim*8], name='g_d3', with_w=True)
            d3 = tf.nn.dropout(self.g_bn_d3(self.d3), 0.5)
            d3 = tf.concat(values=[d3, e5], axis=3)
            # d3 is (8 x 8 x self.gf_dim*8*2)

            self.d4, self.d4_w, self.d4_b = deconv2d(tf.nn.relu(d3),
                [self.batch_size, s16, s16, self.gf_dim*8], name='g_d4', with_w=True)
            d4 = self.g_bn_d4(self.d4)
            d4 = tf.concat(values=[d4, e4], axis=3)
            # d4 is (16 x 16 x self.gf_dim*8*2)

            self.d5, self.d5_w, self.d5_b = deconv2d(tf.nn.relu(d4),
                [self.batch_size, s8, s8, self.gf_dim*4], name='g_d5', with_w=True)
            d5 = self.g_bn_d5(self.d5)
            d5 = tf.concat(values=[d5, e3], axis=3)
            # d5 is (32 x 32 x self.gf_dim*4*2)

            self.d6, self.d6_w, self.d6_b = deconv2d(tf.nn.relu(d5),
                [self.batch_size, s4, s4, self.gf_dim*2], name='g_d6', with_w=True)
            d6 = self.g_bn_d6(self.d6)
            d6 = tf.concat(values=[d6, e2], axis=3)
            # d6 is (64 x 64 x self.gf_dim*2*2)

            self.d7, self.d7_w, self.d7_b = deconv2d(tf.nn.relu(d6),
                [self.batch_size, s2, s2, self.gf_dim], name='g_d7', with_w=True)
            d7 = self.g_bn_d7(self.d7)
            d7 = tf.concat(values=[d7, e1], axis=3)
    ...
    def test(self, args):
        ...
        # sort testing input
        n = [int(i) for i in map(lambda x: x.split(os.sep)[-1].split('.jpg')[0], sample_files)]
    ...
    def create_inception_graph(self, input_tensor):
        with gfile.FastGFile(os.path.join('imagenet', 'classify_image_graph_def.pb'), 'rb') as f:

            graph_def = tf.GraphDef()
            graph_def.ParseFromString(f.read())

            return tf.import_graph_def(graph_def, input_map= {'Mul:0': input_tensor},
                return_elements=['pool_3:0'], name='inception')

    def create_classifier(self, input_tensor):
        with gfile.FastGFile(os.path.join('tf_model', 'op_classify_model_5000.pb'), 'rb') as f:
            graph_def = tf.GraphDef()
            graph_def.ParseFromString(f.read())
            return tf.import_graph_def(graph_def, input_map= {'x_:0': input_tensor},
                return_elements=['op_prediction:0'], name='classifier')
